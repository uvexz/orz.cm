...
If you prefer reviewing SQL before applying it, run `bun run db:generate` and apply the generated files from the `drizzle/` directory with your database tool.

### Add the AUTH_SECRET Environment Variable

The `AUTH_SECRET` environment variable is used to sign Better Auth sessions and verification flows.
